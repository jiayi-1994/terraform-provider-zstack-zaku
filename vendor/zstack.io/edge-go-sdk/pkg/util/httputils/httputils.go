...
import (
	...
	"zstack.io/edge-go-sdk/pkg/util/gotypes"
...
type JSONClientError struct {
	...
	ErrorDetail JSONClientDetailError `json:"error,omitempty"`
	Code        int                   `json:"code,omitzero"`
	Class       string                `json:"class,omitempty"`
	Details     string                `json:"details,omitempty"`
	Causes      *string               `json:"causes,omitempty"`
}
type JSONClientDetailError struct {
...
func JSONRequest(
	...
	method THttpMethod,
	urlStr string,
	//getHeader func(url, method string) (http.Header, error),
	body jsonutils.JSONObject,
	debug bool,
) (http.Header, http.Header, jsonutils.JSONObject, error) {

	//header, err := getHeader(urlStr, string(method))
	//if err != nil {
	...
	respHeader, jsonResult, err := ParseJSONResponse(bodystr, resp, debug, nil)
	if err != nil {
		return nil, nil, nil, err
	}
	return header, respHeader, jsonResult, nil
}
...
func ParseJSONResponse(reqBody string, resp *http.Response, debug bool, autoLoginFunc func() (http.Header, jsonutils.JSONObject, error)) (http.Header, jsonutils.JSONObject, error) {
	defer CloseResponse(resp)
	if debug {
		dump, _ := httputil.DumpResponse(resp, false)
		if resp.StatusCode < 300 {
	...
	if resp.StatusCode < 300 {
		...
		if ce.Class == "ID.1001" && autoLoginFunc != nil {
